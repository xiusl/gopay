...
func (a *Client) SdkExecute(ctx context.Context, bizContent gopay.BodyMap, method string, authToken ...string) (param string, err error) {
...
// 图片上传
func (a *Client) UploadImage(ctx context.Context, bm gopay.BodyMap, file *util.File, method string) (bs []byte, err error) {
	var (
		bodyStr string
		bodyBs  []byte
		aat     string
	...
	url := baseUrlUtf8 + "&" + param
	bm.Reset()
	bm.SetFormFile("image_content", file)
	httpClient := xhttp.NewClient()
	res, bs, err := httpClient.Type(xhttp.TypeMultipartFormData).Post(url).
		SendMultipartBodyMap(bm).EndBytes(ctx)
	if err != nil {
